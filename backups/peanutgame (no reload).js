...
var factory = 0;
var creationLab = 0;
var generatorFacility = 0;
var productionCenter = 0;
var forest = 0;
var island = 0;
var assemblyYard = 0;
var fusionReactor = 0;
...
//Creating upgrade variables
var peanutPrice = 0;
var peanutProduction = 0;

var enchantedSeeds = 0;
var fasterGrowingSaplings = 0;
var tallerTrees = 0;
var largerFields = 0;
var farmExpansion = 0;
var improvedMachines = 0;
var newTechnology = 0;
var fasterGeneration = 0;
var largerProductionSpace = 0;
var strengthenedBranches = 0;
var privatePeanutYatch = 0;
var xlPeanuts = 0;
var strongerFusion = 0;
var stableOrbit = 0;
var artificialLighting = 0;
var improvedSoil = 0;
var fireProofPeanuts = 0;
var peanutBlackHole = 0;
var galacticTrade = 0;
var universePeanuts = 0;
var omniPeanut = 0;
var expertFarming = 0;
...
var divineBlood = 0;
var unlockCreation = 0;
var light = 0;

//Adding value to class variables

function createClassVariables() {
	...
	sapling = new Item("Peanut Sapling", Number(localStorage.getItem(itemsList[1])), 0.08, 5, "A small tree, containing a few peanuts", "images/peanutgame/sapling.png", "sapling", 5);
	tree = new Item("Peanut Tree", Number(localStorage.getItem(itemsList[2])), 0.6, 20, "A larger tree, containing a lot more peanuts", "images/peanutgame/tree.png", "tree", 5);
	field = new Item("Peanut Field", Number(localStorage.getItem(itemsList[3])), 4, 100, "A field full of peanut trees", "images/peanutgame/field.png", "field", 5);
	farm = new Item("Peanut Farm", Number(localStorage.getItem(itemsList[4])), 30, 450, "An actual peanut farm", "images/peanutgame/farm.png", "farm", 5);
	factory = new Item("Peanut Factory", Number(localStorage.getItem(itemsList[5])), 180, 2000, "A factory producing peanuts", "images/peanutgame/factory.png", "factory", 5);
	seed = new Item("Peanut Seed", Number(localStorage.getItem(itemsList[0])), 0.01, 1, "A single seed, growing a single peanut", "images/peanutgame/seeds.png", "seed", 5);
	creationLab = new Item("Peanut Creation Lab", Number(localStorage.getItem(itemsList[6])), 1000, 9000, "Peanuts are created chemically in this lab", "images/peanutgame/creationLab.png", "creationLab", 5);
	generatorFacility = new Item("Peanut Generator Facility", Number(localStorage.getItem(itemsList[7])), 6000, 45000, "Facility generating peanuts in the thousands", "images/peanutgame/generatorFacility.png", "generatorFacility", 5);
	productionCenter = new Item("Underground Peanut Production Center", Number(localStorage.getItem(itemsList[8])), 40000, 200000, "A giant production center, producing peanuts underground", "images/peanutgame/productionCenter.png", "productionCenter", 5);
	forest = new Item("Peanut Forest", Number(localStorage.getItem(itemsList[9])), 200000, 1000000, "A large forest growing millions of peanuts", "images/peanutgame/forest.png", "forest", 5);
	island = new Item("Private Peanut Island", Number(localStorage.getItem(itemsList[10])), 1200000, 4500000, "A private island for growing peanuts", "images/peanutgame/island.png", "island", 5);
	assemblyYard = new Item("Giant Peanut Assembly Yard", Number(localStorage.getItem(itemsList[11])), 7000000, 20000000, "A giant assembly yard, creating giant peanuts", "images/peanutgame/assemblyYard.png", "assemblyYard", 5);
	fusionReactor = new Item("Peanut Fusion Reactor", Number(localStorage.getItem(itemsList[12])), 40000000, 100000000, "Fuse peanuts together to create more peanuts!", "images/peanutgame/fusionReactor.png", "fusionReactor", 5);
	asteroid = new Item("Peanut Asteroid", Number(localStorage.getItem(itemsList[13])), 250000000, 450000000, "An asteroid made out of peanuts", "images/peanutgame/asteroid.png", "asteroid", 5);
	moon = new Item("Peanut Moon", Number(localStorage.getItem(itemsList[14])), 1300000000, 2500000000, "Ever wanted to grow peanuts on the moon? Well, now you can!", "images/peanutgame/moon.png", "moon", 5);
	planet = new Item("Peanut Planet", Number(localStorage.getItem(itemsList[15])), 7500000000, 10000000000, "An entire planet, just to grow peanuts?", "images/peanutgame/planet.png", "planet", 5);
	star = new Item("Peanut Star", Number(localStorage.getItem(itemsList[16])), 45000000000, 45000000000, "Works like a fusion reactor, but a lot bigger", "images/peanutgame/star.png", "star", 5);
	galaxy = new Item("Peanut Galaxy", Number(localStorage.getItem(itemsList[17])), 260000000000, 230000000000, "A galaxy full of peanut-growing planets", "images/peanutgame/galaxy.png", "galaxy", 5);
	universe = new Item("Peanut Universe", Number(localStorage.getItem(itemsList[18])), 1500000000000, 1350000000000, "How did you even manage to buy this?", "images/peanutgame/universe.png", "universe", 5);
	multiverse = new Item("Peanut Multiverse", Number(localStorage.getItem(itemsList[19])), 9000000000000, 6000000000000, "When the universe isn't big enough to grow peanuts", "images/peanutgame/multiverse.png", "multiverse", 5);
	omniverse = new Item("Peanut Omniverse", Number(localStorage.getItem(itemsList[20])), 55000000000000, 25000000000000, "Could it get even bigger than this?", "images/peanutgame/omniverse.png", "omniverse", 5);
	box = new Item("The Box", Number(localStorage.getItem(itemsList[21])), 280000000000000, 100000000000000, "The Box, containing everything in existence", "images/peanutgame/the box.png", "box", 5);
	theVoid = new Item("The Void", Number(localStorage.getItem(itemsList[22])), 3500000000000000, 1000000000000000, "An infinitely large, empty space", "images/peanutgame/void.png", "void", 5);

	//Creating farmer objects from classes
	shnilli = new Farmer("Shnilli", Number(localStorage.getItem(farmersList[0])), 0.003, 1, "Everyone's favorite chocolate potato", "images/peanutgame/shnilli.png", "shnilli", 3);
	littina = new Farmer("Littina", Number(localStorage.getItem(farmersList[1])), 0.008, 2, "Shnilli's sister, Littina", "images/peanutgame/littina.png", "littina", 5);
	bean = new Farmer("The Bean", Number(localStorage.getItem(farmersList[2])), 0.04, 8, "Smol boi and friend of Shnilli", "images/peanutgame/the bean.png", "bean", 5);
	honey = new Farmer("Honey", Number(localStorage.getItem(farmersList[3])), 0.2, 20, "Actual, living honey about half the size of a stickman", "images/peanutgame/honey.png", "honey", 5);
	farmer = new Farmer("Peanut Farmer", Number(localStorage.getItem(farmersList[4])), 1, 80, "Just a normal peanut farmer", "images/peanutgame/farmer.png", "farmer", 5);
	bot = new Farmer("AbominationBot", Number(localStorage.getItem(farmersList[5])), 8.5, 400, "A bot desiged to <s>defend Abominations</s> farm peanuts", "images/peanutgame/abominationbot.png", "bot", 5);
	cactus = new Farmer("The Cactus", Number(localStorage.getItem(farmersList[6])), 50, 1800, "Who knows better how to survive in harsh environments than a cactus?", "images/peanutgame/cactus.png", "cactus", 5);
	ghp = new Farmer("GHP", Number(localStorage.getItem(farmersList[7])), 400, 10000, "Giant Humanoid Peanut himself, here to help take care of his farm", "images/peanutgame/ghp.png", "ghp", 5);
	overseer = new Farmer("Abomination Overseer", Number(localStorage.getItem(farmersList[8])), 2500, 55000, "The Abomination Overseer, watching over all Abominations", "images/peanutgame/overseer.png", "overseer", 5);
	davz = new Farmer("The Davz", Number(localStorage.getItem(farmersList[9])), 16000, 250000, "Davz himself joins in to farm peanuts", "images/peanutgame/davz.png", "davz", 15);
	pea = new Farmer("The Pea", Number(localStorage.getItem(farmersList[10])), 850000, 12000000, "A giant Abomination, even bigger than the Stickworld itself", "images/peanutgame/the pea.png", "pea", 25);
	penut = new Farmer("Holy Penut", Number(localStorage.getItem(farmersList[11])), 500000000, 2800000000, "The god of peanuts, chillness and peace", "images/peanutgame/holy penut.png", "penut", 20);
	bread = new Farmer("The Bread", Number(localStorage.getItem(farmersList[12])), 73000000000, 200000000000, "An Abomination the size of the sun", "images/peanutgame/bread.png", "bread", 5);
	theGalaxy = new Farmer("The Galaxy", Number(localStorage.getItem(farmersList[13])), 500000000000, 1500000000000, "A living galaxy, twice the size of the Milky Way", "images/peanutgame/theGalaxy.png", "theGalaxy", 20);
	maggot = new Farmer("The Maggot", Number(localStorage.getItem(farmersList[14])), 140000000000000, 150000000000000, "A completely normal maggot, 200 times the size of the Omniverse", "images/peanutgame/maggot.png", "maggot", 5);
	abominodas = new Farmer("Abominodas", Number(localStorage.getItem(farmersList[15])), 700000000000000, 650000000000000, "One of the most powerful Abomination Gods", "images/peanutgame/abominodas.png", "abominodas", 5);
	theInception = new Farmer("The Inception", Number(localStorage.getItem(farmersList[16])), 5800000000000000, 5000000000000000, "The first, the last, the strongest", "images/peanutgame/inception.png", "creation", 5);
	...
	peanutPrice = new Upgrade("Bigger Peanuts", Number(localStorage.peanutPrice), 15, 0.25, "Increases the value of peanuts from $0.001 to $0.0011", "images/peanutgame/upgrades/peanut.png", "peanutPrice", "peanutValue");
	peanutProduction = new Upgrade("Increased Production", Number(localStorage.peanutProduction), 12, 0.5, "Increases the amount of peanuts produced by everything by a total of 10%", "images/peanutgame/upgrades/production.png", "peanutProduction", "peanutProduction");

	enchantedSeeds = new Upgrade("Enchanted Seeds", Number(localStorage.getItem(itemUpgradeList[0])), 1, 0.1, "The Peanut Seeds get enchanted, doubling their peanut production", "images/peanutgame/upgrades/enchantedSeeds.png", "enchantedSeeds", "itemUpgrade");
	fasterGrowingSaplings = new Upgrade("Faster-Growing Saplings", Number(localStorage.getItem(itemUpgradeList[1])), 1, 0.8, "The Peanut Saplings grow faster, doubling their peanut production", "images/peanutgame/sapling.png", "fasterGrowingSaplings", "itemUpgrade");
	tallerTrees = new Upgrade("Taller Trees", Number(localStorage.getItem(itemUpgradeList[2])), 1, 6, "The Peanut Trees get taller, doubling their peanut production", "images/peanutgame/tree.png", "tallerTrees", "itemUpgrade");
	largerFields = new Upgrade("Larger Fields", Number(localStorage.getItem(itemUpgradeList[3])), 1, 40, "The Peanut Fields become larger, doubling their peanut production", "images/peanutgame/field.png", "largerFields", "itemUpgrade");
	farmExpansion = new Upgrade("Farm Expansion", Number(localStorage.getItem(itemUpgradeList[4])), 1, 300, "The Peanut Farms get expanded, doubling their peanut production", "images/peanutgame/farm.png", "farmExpansion", "itemUpgrade");
	improvedMachines = new Upgrade("Improved Machines", Number(localStorage.getItem(itemUpgradeList[5])), 1, 1800, "The Peanut Factories get improved machines, doubling their peanut production", "images/peanutgame/factory.png", "improvedMachines", "itemUpgrade");
	newTechnology = new Upgrade("New Technology", Number(localStorage.getItem(itemUpgradeList[6])), 1, 10000, "The Peanut Creation Labs discovers new technology, doubling their peanut production", "images/peanutgame/creationLab.png", "newTechnology", "itemUpgrade");
	fasterGeneration = new Upgrade("Faster Generation", Number(localStorage.getItem(itemUpgradeList[7])), 1, 60000, "The Peanut Generator Facilities doubles their generation speed, which also doubles their peanut production", "images/peanutgame/generatorFacility.png", "fasterGeneration", "itemUpgrade");
	largerProductionSpace = new Upgrade("Larger Production Space", Number(localStorage.getItem(itemUpgradeList[8])), 1, 400000, "The Underground Peanut Production Centers gets larger production spaces, doubling their peanut production", "images/peanutgame/productionCenter.png", "largerProductionSpace", "itemUpgrade");
	strengthenedBranches = new Upgrade("Strengthened Branches", Number(localStorage.getItem(itemUpgradeList[9])), 1, 2000000, "The Peanut Forests get strengthened branches, doubling their peanut production", "images/peanutgame/forest.png", "strengthenedBranches", "itemUpgrade");
	privatePeanutYatch = new Upgrade("Private Peanut Yatch", Number(localStorage.getItem(itemUpgradeList[10])), 1, 12000000, "The Private Peanut Islands get their own Private Peanut Yatchs, doubling their peanut productions", "images/peanutgame/island.png", "privatePeanutYatch", "itemUpgrade");
	xlPeanuts = new Upgrade("XL Peanuts", Number(localStorage.getItem(itemUpgradeList[11])), 1, 70000000, "The Giant Peanut Assembly Yards now produce even larger peanuts, doubling their peanut production", "images/peanutgame/assemblyYard.png", "xlPeanuts", "itemUpgrade");
	strongerFusion = new Upgrade("Stronger Fusion", Number(localStorage.getItem(itemUpgradeList[12])), 1, 400000000, "The Peanut Fusion Reactos achieves stronger fusion, doubling their peanut production", "images/peanutgame/fusionReactor.png", "strongerFusion", "itemUpgrade");
	stableOrbit = new Upgrade("Stable Orbit", Number(localStorage.getItem(itemUpgradeList[13])), 1, 2500000000, "The Peanut Asteroids get a stable orbit, doubling their peanut production", "images/peanutgame/asteroid.png", "stableOrbit", "itemUpgrade");
	artificialLighting = new Upgrade("Artificial Lighting", Number(localStorage.getItem(itemUpgradeList[14])), 1, 13000000000, "The Peanut Moons build artifical lighting to double their peanut production", "images/peanutgame/moon.png", "artificialLighting", "itemUpgrade");
	improvedSoil = new Upgrade("Improved Soil", Number(localStorage.getItem(itemUpgradeList[15])), 1, 75000000000, "The soil of the Peanut Planets gets improved, doubling their peanut production", "images/peanutgame/planet.png", "improvedSoil", "itemUpgrade");
	fireProofPeanuts = new Upgrade("Fire-Proof Peanuts", Number(localStorage.getItem(itemUpgradeList[16])), 1, 450000000000, "The Peanut Stars now produce fire-proof peanuts, doubling their peanut production", "images/peanutgame/star.png", "fireProofPeanuts", "itemUpgrade");
	peanutBlackHole = new Upgrade("Peanut Black Hole", Number(localStorage.getItem(itemUpgradeList[17])), 1, 2600000000000, "A Peanut Black Hole is created in the Peanut Galaxies, doubling their peanut production", "images/peanutgame/galaxy.png", "peanutBlackHole", "itemUpgrade");
	galacticTrade = new Upgrade("Inter-Galactic Trade", Number(localStorage.getItem(itemUpgradeList[18])), 1, 15000000000000, "Inter-galactic trade is now established in the Peanut Universes, doubling their peanut production", "images/peanutgame/universe.png", "galacticTrade", "itemUpgrade");
	universePeanuts = new Upgrade("Universe-Sized Peanuts", Number(localStorage.getItem(itemUpgradeList[19])), 1, 90000000000000, "The Peanut Multiverses now produce universe-sized peanuts, doubling their peanut production", "images/peanutgame/multiverse.png", "universePeanuts", "itemUpgrade");
	omniPeanut = new Upgrade("Omni-Peanut", Number(localStorage.getItem(itemUpgradeList[20])), 1, 550000000000000, "The peanuts produced in the Peanut Omniverses are now omnipotent. This also doubles their peanut production", "images/peanutgame/omniverse.png", "omniPeanut", "itemUpgrade");
	expertFarming = new Upgrade("Expert Farming", Number(localStorage.getItem(itemUpgradeList[21])), 1, 2800000000000000, "The Expert helps farming peanuts, doubling the peanut production of The Box", "images/peanutgame/upgrades/expert.png", "expertFarming", "itemUpgrade");

	tinyArmor = new Upgrade("Tiny Armor", Number(localStorage.getItem(farmerUpgradeList[0])), 1, 0.03, "Shnilli gets armor, doubling his peanut production", "images/peanutgame/upgrades/armorShnilli.png", "tinyArmor", "farmerUpgrade");
	reckoning = new Upgrade("Day of Reckoning", Number(localStorage.getItem(farmerUpgradeList[1])), 1, 0.4, "Littina grows dark blades, tripling her peanut production", "images/peanutgame/upgrades/reckoning.png", "reckoning", "farmerUpgrade");
	vines = new Upgrade("Vines from Below", Number(localStorage.getItem(farmerUpgradeList[2])), 1, 0.4, "The Bean transforms into its Inner Bean form, doubling its peanut production", "images/peanutgame/upgrades/innerBean.png", "vines", "farmerUpgrade");
	metallicLimbs = new Upgrade("Metallic Limbs", Number(localStorage.getItem(farmerUpgradeList[3])), 1, 2, "Honey uses its stickbot suit to double its peanut production", "images/peanutgame/upgrades/honeybot.png", "metallicLimbs", "farmerUpgrade");
	pitchfork = new Upgrade("Peanut Pitchfork", Number(localStorage.getItem(farmerUpgradeList[4])), 1, 10, "The Peanut Farmer uses a peanut pitchfork to double its peanut production", "images/peanutgame/upgrades/farmer.png", "pitchfork", "farmerUpgrade");
	botUpgrade = new Upgrade("Bot Upgrade", Number(localStorage.getItem(farmerUpgradeList[5])), 1, 85, "The AbominationBot gets upgraded, doubling its peanut production", "images/peanutgame/upgrades/bot v2.png", "botUpgrade", "farmerUpgrade");
	flowers = new Upgrade("Desert Flowers", Number(localStorage.getItem(farmerUpgradeList[6])), 1, 500, "The Cactus grows flowers, somehow doubling its peanut production", "images/peanutgame/upgrades/cactus.png", "flowers", "farmerUpgrade");
	grp = new Upgrade("GRP", Number(localStorage.getItem(farmerUpgradeList[7])), 1, 4000, "GHP becomes a robot, doubling his peanut production", "images/peanutgame/upgrades/grp.png", "grp", "farmerUpgrade");
	farmingMagic = new Upgrade("Farming Magic", Number(localStorage.getItem(farmerUpgradeList[8])), 1, 25000, "The Abomination Overseer learns farming magic, doubling its peanut production", "images/peanutgame/overseer.png", "farmingMagic", "farmerUpgrade");
	peanutStabber = new Upgrade("Peanut Stabber", Number(localStorage.getItem(farmerUpgradeList[9])), 1, 160000, "The Davz stabs the peantuts to farm them twice as fast", "images/peanutgame/davz.png", "peanutStabber", "farmerUpgrade");
	heightIncrease = new Upgrade("Height Increase", Number(localStorage.getItem(farmerUpgradeList[10])), 1, 8500000, "The Pea grows even taller, doubling its peanut production", "images/peanutgame/the pea.png", "heightIncrease", "farmerUpgrade");
	penutAura = new Upgrade("Penut Aura", Number(localStorage.getItem(farmerUpgradeList[11])), 1, 5000000000, "The Holy Penut gets a Penut Aura, doubling its peanut production", "images/peanutgame/upgrades/holy penut.png", "heightIncrease", "farmerUpgrade");
	fleshBlobs = new Upgrade("Arrival of the Flesh-Blobs", Number(localStorage.getItem(farmerUpgradeList[12])), 1, 730000000000, "The Flesh-Blobs help The Bread farming, doubling its production", "images/peanutgame/bread.png", "fleshBlobs", "farmerUpgrade");
	lightspeed = new Upgrade("Lightspeed Farming", Number(localStorage.getItem(farmerUpgradeList[13])), 1, 5000000000000, "The Galaxy's speed increases dramatically, doubling its peanut production", "images/peanutgame/theGalaxy.png", "lightspeed", "farmerUpgrade");
	duplication = new Upgrade("Maggot Duplication", Number(localStorage.getItem(farmerUpgradeList[14])), 1, 1400000000000000, "The Maggot duplicates, doubling its peanut production", "images/peanutgame/maggot.png", "duplication", "farmerUpgrade");
	power = new Upgrade("Unlimited Power", Number(localStorage.getItem(farmerUpgradeList[15])), 1, 7000000000000000, "Abominodas transforms into The Abominodas, gaining near unlimited power and doubling his peanut production", "images/peanutgame/upgrades/the abominodas.png", "power", "farmerUpgrade");

	unlockVoid = new Upgrade("Void", Number(localStorage.unlockVoid), 1, 1000000000000000, "Void", "images/peanutgame/void.png", "unlockVoid", "itemUpgrade");
	darkness = new Upgrade("Darkness", Number(localStorage.getItem(itemUpgradeList[22])), 1, 35000000000000000, "The Void gets filled by darkness...", "images/peanutgame/void.png", "darkness", "itemUpgrade");

	divineBlood = new Upgrade("Divine Blood", Number(localStorage.getItem("Divine Blood")), 1, 0.3, "Shnilli transforms into Divine Shnilli, doubling his peanut production further", "images/peanutgame/upgrades/divine shnilli.png", "divineBlood", "farmerUpgrade");
	unlockCreation = new Upgrade("Creation", Number(localStorage.unlockCreation), 1, 1500000000000000, "Creation", "images/peanutgame/upgrades/light.png", "unlockCreation", "farmerUpgrade");
	light = new Upgrade("Light of Creation", Number(localStorage.getItem(farmerUpgradeList[16])), 1, 58000000000000000, "An immense light surrounds The Inception...", "images/peanutgame/upgrades/inception.png", "light", "farmerUpgrade");
}
